...
import {getControlRoutes_url} from './getcontrolroutes_url'
...
export async function initControlRoutes() {
    ...
    // 获取路由菜单数据
    const res = await getControlRoutes();
    // 无登录权限时，添加判断
 ...
 * @returns 返回后端路由菜单数据
 */
export async function getControlRoutes() {
    const url = await getControlRoutes_url();
