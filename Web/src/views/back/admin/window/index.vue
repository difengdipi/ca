<template>
	<div style="margin: 20px">
		<el-input v-model="searchQuery.name" placeholder="请输入名称" style="width: 300px; margin-right: 10px" clearable> </el-input>

		<el-input v-model="searchQuery.description" placeholder="请输入描述" style="width: 300px; margin-right: 10px" clearable> </el-input>
		...
		<el-table :data="data" style="width: 100%; margin-top: 20px" stripe border>
			...
			>
			<el-table-column prop="business_hours" label="营业时间"></el-table-column>
		...
		<el-dialog v-model="dialogVisible" title="数据表单" width="30%">
			<el-form :model="form" label-width="100px">
				<el-form-item label="食堂">
					<el-select v-model="form.canteen_id" placeholder="请选择食堂">
						<el-option v-for="(item, index) in Canteen_Datas" :key="index" :label="item.name" :value="item.id" />
					</el-select>
				</el-form-item>
...
const searchData = async () => {
	try {
		...
		const response = await axios.post('/api/search/window/', searchQuery.value, { params });
...
const Canteen_Datas = ref([]);
const get_Canteen_Datas = async () => {
	const res = await axios.get('/api/canteen/', { page: 1, page_size: 999 });
	Canteen_Datas.value = res.data.results;
};
