...
def generate_recommendations(user_id):
    ...
    try:
        df = pd.read_csv('campus_food_ratings.csv')
    except FileNotFoundError:
        return "<p style='color: red;'>错误：找不到 campus_food_ratings.csv 文件。</p>"
...
def submit_input(user_id):
    return generate_recommendations(user_id)

with gr.Blocks(css="#input_box {padding: 20px;}") as demo:
    gr.Markdown("# 校园食堂菜品推荐系统")
    gr.Markdown("输入用户ID，点击按钮生成推荐。")

    user_id = gr.Textbox(lines=1, placeholder="输入用户ID", label="用户ID")
    ...
    submit_button.click(fn=submit_input, inputs=user_id, outputs=output)
    clear_button.click(fn=clear_input, inputs=[], outputs=user_id)
